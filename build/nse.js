...
const precisionRound = _require.precisionRound;

var _require2 = require('mathjs');

const mean = _require2.mean;
...
const calcNse = input => {
  const dataType1WithPrediction = input.dataType1WithPrediction,
        ...
        runoff_trans_mean_mm = input.runoff_trans_mean_mm;
...
const formatNse = (f, area) => {
  ...
  for (let id in swTestStats) {
    ...
    const meanObserveDrain = meanObserveDrainArr.length > 0 ? mean(meanObserveDrainArr) : NaN;

    const meanObserveRunoffArr = dataPoints.map(d => d[observeRunoffKeyY]).filter(d => d !== undefined);

    const meanObserveRunoff = meanObserveRunoffArr.length > 0 ? mean(meanObserveRunoffArr) : NaN;
    ...
    tests.push(Object.assign({}, swTestStats[id], {
