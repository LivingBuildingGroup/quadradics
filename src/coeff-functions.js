...
const createFunctionFromCoeffData = data => {
  ...
  const polyArr = Array.isArray(data.poly) ?
    data.poly : [];

  function poly(x){
    let y = 0;
    polyArr.forEach(segment=>{
      if(isPrimitiveNumber(segment.k) && 
        isPrimitiveNumber(segment.n)){
  ...
  function exp(x){
    if(isPrimitiveNumber(data.k) && 
        isPrimitiveNumber(data.n)){
  ...
  rows.forEach(join=>{
    ...
    if(Array.isArray(join.names_versions)){
      join.names_versions.forEach(v=>{
        ...
        coeffs[join.join_type][v][join.profile_code][coeff.functionType] = 
					Object.assign({},
					  coeff,
					  boilerplate
					);
...
const baseCodesToTry = [
  ...
  'profile_code_20_t', // same snake
];

const profileCodesToTry = [
  'profileCodeExact', // best fit
  'profile_code_exact',
  'profileCodeExactNvg',
...
const createCoeffData = (joinType, thisProfile, coeffKey, versions) => {
  ...
  codesToTry.forEach(c=>{
    ...
    if(profileCode){ // don't check if not found
      _versions.forEach(v=>{ 
        // versions should list either only the current version, or list all versions from preferable to least preferable
        if(!data){ // if already found (better fit), don't look anymore
          if(coeffKey[joinType] &&
            coeffKey[joinType][v] &&
            coeffKey[joinType][v][profileCode]){

            const thisData = 
						coeffKey[joinType][v][profileCode];

            data =
							thisData.poly && !Array.isArray(thisData.poly) ?
							  thisData.poly :
							  thisData.exp ?
							    thisData.exp :
							    thisData.power ?
